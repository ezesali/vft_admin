...
export function AddRestaurant() {
  ...
  const docRefLast = query(collection(db, "restaurant_master"));
  ...
  const [loading, setLoading] = useState(false)
    ...
    const submitAdd = async() => {
        ...
        restaurants.address == '' ) {
            ...
            const docData = {
                restaurant_id   : lastId+1,
                city_id         : restaurants.city_id,
                tour_id         : restaurants.tour_id,
                name            : restaurants.restaurantName,
                url             : imageURL, 
                image           : imageUpload,
                mobile          : restaurants.mobile.toString(),
                longitude       : longitude,
                latitude        : latitude,
                position        : parseInt(restaurants.position),
            ...
            setDoc(doc(db, "restaurant_master", ''+parseInt(lastId+1)+''), docData).then((res) =>{
                ...
                setMessageAlert('Restaurant Add Successfully')
                setOpenAlert(true)
                setRestaurants({})
        ...
        <div style={{marginLeft: navisOpen ? '250px' : null}} className="dashboardContainerProfile">
                    ...
                    <div className="profileHead">
                        <nav aria-label="breadcrumb" role="navigation">
                            <ol className="breadcrumb">
                                ...
                                <>&nbsp;&nbsp;&gt;&nbsp;&nbsp;</>
                                <li className="breadcrumb-item active" aria-current="page">Add Restaurant</li>
                            </ol>
                        </nav>
                    </div>
                </div>
            ...
            <div className="rowViewCity">
                <div className="titleCityEditAdd">
                    <h4 className="editCityH4">Add Restaurant</h4>
                </div>
                ...
                onClick={() => submitAdd()} className="btnSubmitCity" type="submit">
                    {loadingUpdate ? <LoadingSpinner styleContainer={{height: '24px', position: 'relative', backgroundColor: 'none', background: 'none'}} styleLoad={{width: '10px', height: '10px', borderWidth: '5px'}}/> : 'Add Restaurant'}
                </button>
