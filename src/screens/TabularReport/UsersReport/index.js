...
export function UsersReport() {
  ...
  const docRef = query(collection(db, "user_master"), orderBy("createtime","desc"), where("delete_flag", "==", 0), where("user_type", "==", 1), where("profile_complete","==",1));
...
const getUsersData = async() => {
    ...
    const exportToExcel = async(excelData, fileName) => {
        ...
        userExport = userExport.map((val) => {
            return {
                ...
                'Email': val.email,
                'Registered Date & Time': val.regDate,
                'Status': val.status === 1 ? 'Activate' : 'Desactivate'
            }
        })
        ...
        const wb = { Sheets: { 'User Report': ws }, SheetNames: ['User Report'] };
    ...
    const actions = [
        ...
        { 
            ...
            isFreeAction: true,
            onClick: (event) => exportToExcel(users, 'VFT_Users_Report')
        }
    ]
    ...
    <>
        ...
        <div style={{marginLeft: navisOpen ? '180px' : null}} className="dashboardContainerProfile">
            ...
            <div className="contentDate"> 
                ...
                <h3 className="reportName">Users Report</h3>
            ...
            {users.length > 0 ?
                <div className="tableView">
                    <Table columns={columns} rows={users} actions={actions} title={'User Report List'}/>
                </div>
            :
             
                <div className="tableView">
                    <p className="reportMessage">There are not users created between those Dates</p>
                </div>
            }
