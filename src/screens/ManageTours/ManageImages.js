...
export function ManageTourImages() {
  ...
  const [tourImgDesc, setTourImgDesc] = useState('')

  const [lastId, setLastId] = useState(null)


  const docRef = query(collection(db, "tour_image_master"));
  ...
  const {id} = useParams()
    ...
    const handleChangeCityImage = (event) => {
        if (event.target.files && event.target.files[0]) {
            ...
            setTourImg(event.target.files[0])
    ...
    const submitAdd = async() => {
        ...
        let imageURL = tourImg

        if(imageURL){

            const fileRefSmall = ref(storage, 'images/'+  tourImg.name+ '.png');

            await uploadBytes(fileRefSmall, tourImg)
        ...
        const docData = {
            tour_img_id    : lastId+1,
            tour_id        : parseInt(id),
            url            : imageURL,
            image          : imageUpload,
            description    : tourImgDesc,
            delete_flag    : 0,
        ...
        setDoc(doc(db, "tour_image_master", ''+parseInt(lastId+1)+''), docData).then((res) =>{
                    ...
                    <div className="profileHead">
                        <nav aria-label="breadcrumb" role="navigation">
                            <ol className="breadcrumb">
                                ...
                                <>&nbsp;&nbsp;&gt;&nbsp;&nbsp;</>
                                <li className="breadcrumb-item active" aria-current="page">Upload Tour Image</li>
                            </ol>
                        </nav>
                    </div>
                </div>
            ...
            <div className="rowViewCity">
                ...
                <div className="cityInputs">
                    ...
                    <div className="viewImageCity">
                        ...
                        <img
                            src={imageUpload ? imageUpload : tourImg ? tourImg : NoCity}
                            alt={''}
                    ...
                    <div className="cityInputs">
                        ...
                        <TextField InputLabelProps={{shrink: true}} onChange={onChangeDescription} defaultValue={tourImgDesc} multiline label="Description (optional)" placeholder="Enter Description" id="outlined-basic" variant="outlined" />
